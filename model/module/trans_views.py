...
class SRM_Block(nn.Module):
    ...
    def forward(self, x_1, x_2):
        #print("in SRM:",x_1.shape)
        #print("att:",(self.attn_1(self.norm1_1(x_1))).shape)
        x_1 = x_1 + self.drop_path(self.attn_1(self.norm1_1(x_1)))
        x_2 = x_2 + self.drop_path(self.attn_2(self.norm1_2(x_2)))
       
        # print("in SRM:",x_1.shape)#orch.Size([256, 351, 512])
        x = torch.cat([x_1, x_2], dim=2)
        #print("concate:", x.shape)#torch.Size([256, 351, 1536])
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        #print("norm drop :", self.mlp(self.norm2(x)).shape)
        x_1 = x[:, :, :x.shape[2] // 2]
        x_2 = x[:, :, x.shape[2] // 2: x.shape[2] // 2 * 2]
        
        #print("iend:", x_1.shape,x_2.shape)
        return x_1, x_2#, x_3


class IFM_Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_hidden_dim, qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
...
class Transformer(nn.Module):
    ...
    def forward(self, x_1, x_2):
        #print("in refinement block \n")
        #print('x_1', x_1.shape, 'x_2', x_2.shape)
        ...
        #print('next x_1', x_1.shape, 'x_2', x_2.shape)
        for i, blk in enumerate(self.SRM_blocks):
            x_1, x_2= self.SRM_blocks[i](x_1, x_2)
        #print('after SRM x_1', x_1.shape, 'x_2', x_2.shape)
        x_1, x_2= self.IFM_blocks[0](x_1, x_2)

        x = torch.cat([x_1, x_2], dim=2)
        #print("before regression:", x.shape)
        x = self.norm(x)
